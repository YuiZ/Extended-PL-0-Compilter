...
int Block_sign[MidCode_Max_Size];//基本快划分标记
...
void SubProgram::print_blocks()
{
    ...
    for (int i = 0; i < Blocks_Size; ++i)
    {
        ...
        //Blocks[i]->print_sets(P_vn);//打印USE DEF IN OUT集合计算结果
    }
...
void opt_divide2block()//划分基本块
{
    ...
    for (int i = 0; i < P_qc; ++i)
    {
        ...
        {
            Block_sign[i + 1] = 1;//跳转后的第一句
            Block_sign[atoi(QuatriCode[i]->n1.c_str())] = 1;//跳转目标
        }
        else if (QuatriCode[i]->op == "ENTER")//过程入口
            Block_sign[i] = 1;
        else if (QuatriCode[i]->op == "CALL")//过程调用返回位置
        {
            ...
            if (QuatriCode[i+j]->op != "RETURN_TO")//若有RETURN_TO则是函数调用，函数调用出现在表达式中间，不应打断
                Block_sign[i+j] = 1;
        }
    }
...
void opt_divede2proc()
{
    ...
    while (i < P_qc)
    {
        if (QuatriCode[i]->op == "ENTER")
        {
            ...
            SubProg[P_sp]->build_blocks();//对每个分程序，建立其基本块记录
...
void Opt_process()
{
    opt_divide2block();//划分基本块
    opt_divede2proc();//划分分程序

    mips_reg = new GPR(8, Temp_Reg_Size);//设定寄存器堆
    opt_temp_reg_distribute();//临时寄存器预分配处理
